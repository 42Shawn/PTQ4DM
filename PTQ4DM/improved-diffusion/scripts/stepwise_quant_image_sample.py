"""
Calibrate the activation for each timestep
"""
...
from QDrop.quant.quant_layer import UniformAffineQuantizer
import torch
...
def stepwise_calibration(
    raw_model, qnn, args, num_samples, device, t_mode, diffusion, class_cond=True
):
    model_kwargs = {}
    if class_cond:
        cls = torch.tensor([1] * num_samples, device=device).long()  # TODO class gen
    ...
    for now_rt, sample_t in enumerate(
        ...
        st = time.time()
        now_input = sample_t["input"]
        for name, module in qnn.named_modules():
            if (
                isinstance(module, (QuantModule, BaseQuantBlock))
                and module.act_quantizer.timewise_quant_mode
            ):
                module.act_quantizer.change_timestep(
                    diffusion.num_timesteps - now_rt - 1
                )
        t = torch.tensor(
            [diffusion.num_timesteps - now_rt - 1] * num_samples, device=device
        )
        t = diffusion._scale_timesteps(t)
        cali_data = now_input, t
        set_act_quantize_params(
            qnn, cali_data=cali_data, awq=args.awq, order=args.order
        )
        ed = time.time()
        print(
            f"calibration for timestep={diffusion.num_timesteps-now_rt-1} takes {ed-st:.2f}s"
        )
...
def quant_model(args, model, diffusion):
    ...
    for name, module in qnn.named_modules():
        if isinstance(module, (QuantModule)):
            print(f"set {name} weight fast mode")
            module.weight_quantizer.fast_mode = True
    # sample weight calib data
    print("sampling weight calib data")
    ...
    print("calibrate activation")
    for name, module in qnn.named_modules():
        if isinstance(module, (QuantModule, BaseQuantBlock)):
            # if isinstance(module, UniformAffineQuantizer):
            print(f"set {name} act fast mode and timestep mode")
            module.act_quantizer.fast_mode = True
            module.act_quantizer.timewise_quant_mode = True
    if not args.use_adaround:
        stepwise_calibration(
            raw_model,
            qnn,
            args,
            args.calib_num_samples,
    ...
    while len(all_images) * args.batch_size < args.num_samples:
        ...
        if 1:
            ...
            if args.out_path == "":
                ...
                out_path = f"outputs/stepwise_samples_{args.calib_im_mode}_{args.calib_t_mode}_{shape_str}_{args.calib_num_samples}.npz"
            else:
...
def create_argparser():
    ...
    parser.add_argument(
        ...
        default="random",
        type=str,
        choices=["random", "1", "-1", "mean", "uniform"],
    )
